...
interface Props {
  ...
  searchParams?: Promise<{ ref?: string; market?: string }>;
}
...
export default async function RegisterPage({ searchParams }: Props) {
  ...
  const defaultMarket  = (params?.market ?? "").trim();
  ...
  return (
    <main className="min-h-screen bg-ap-bg flex flex-col items-center justify-center p-5">
      ...
      <RegisterPageClient defaultRef={defaultRef} defaultMarket={defaultMarket} banks={banks} logoUrl={logoUrl} />
    </main>
  );
}
